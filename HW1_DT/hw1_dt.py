...
class TreeNode(object):
    ...
    def split(self):
        if self.splittable:
            ...
            print(len(self.feature_uniq_split),'=',len(best_labels))
            # build child node
            ...
            for i in range(0,len(best_labels)):
                ...
                if len(child_features[0]) <= 0:
                    ...
                    child.split()
                children_sort_info[child] = self.feature_uniq_split[i]
            children_sorted_info = sorted(children_sort_info.items(),key = lambda x: x[1])
    ...
    def predict(self, feature):
        ...
        feature = np.delete(feature,self.dim_split,axis = 0).tolist()
